...
%% Macros
-define(SERVER, mysql_dispatcher_log).
...
async_exec(PoolId, Query) ->
    gen_server:cast(?SERVER, {async_exec, PoolId, Query}).
...
handle_cast({async_exec, PoolId, Query}, State) ->
    try async_exec_queries(PoolId, State, [Query])
    catch T:X ->
        io:format("mysql async_exec error: ~p : ~p\n", [T, X]),
        {noreply, State}
    end;
...
async_exec_queries(PoolId, State, QueryList) ->
    case with_next_conn(
      PoolId, State,
      fun(Conn, State1) ->
	      Pid = Conn#conn.pid,
	      mysql_conn:async_exec(Pid, QueryList),
	      {noreply, State1}
      end) 
        of
