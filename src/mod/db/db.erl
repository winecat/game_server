...
%%     ?profile_start,
%%     Result = 
%%         case emysql:execute(?DB_SYNC_POOL, Sql) of
%%             {updated, #ok_packet{affected_rows = R}} -> R;
%%             {error, #result_packet{error = Reason}} -> mysql_halt([Sql, Reason]);
...
%% %% 使用方法：db:execute("UPDATE role SET coin=~s WHERE name=~s", [10000, "test"]).
%% %% </pre>
%% execute(Sql, Args) ->
%%     Query = format_sql(Sql, Args),
%%     ?profile_start,
%%     Result = case mysql:fetch(?DB_SYNC_POOL, Query) of
...
%%     Query = format_sql(Sql, Args),
%%     ?profile_start,    
%%     Result = try mysql:fetch(?DB_SYNC_POOL, Query) of
%%         _Any -> ok
%%     catch
%%         _:_ -> error
%%     end,
...
%% cast(Sql) ->
%%     ?profile_start,
%%     mysql:async_exec(?DB_ASYNC_POOL, Sql),
%%     ?profile(Sql),
%%     ok.
%% 
%% %% -> ok
...
%%     Query = format_sql(Sql, Args),
%%     ?profile_start,
%%     mysql:async_exec(?DB_ASYNC_POOL, Query),
%%     ?profile(Sql),
%%     ok.
%% 
...
%% spec_execute(Db, Sql) ->
%%     ?profile_start,
%%     Result = case mysql:fetch(Db, Sql) of
%%         {updated, #mysql_result{affectedrows = R}} -> R;
...
%%     Query = format_sql(Sql, Args),
%%     ?profile_start,
%%     Result = case mysql:fetch(Db, Query) of
%%         {updated, #mysql_result{affectedrows = R}} -> R;
%%         {error, #mysql_result{error = Reason}} -> mysql_halt([Query, Reason]);
%%         {error, _Reason} -> erlang:error({db_error, [_Reason]})
%%     end,
...
%%     ?profile_start,
%%     Result = case mysql:fetch(?DB_SYNC_POOL, S) of
...
%%     ?profile_start,
%%     mysql:prepare(s, S),
%%     Result = case mysql:execute(?DB_SYNC_POOL, s, Args) of
%%         {data, #mysql_result{rows = R}} -> R;
%%         {error, #mysql_result{error = Reason}} -> mysql_halt([Sql, Reason]);
%%         {error, _Reason} -> erlang:error({db_error, [_Reason]})
%%     end,
...
%% get_one(Sql) ->
%%     ?profile_start,
%%     Result = case mysql:fetch(?DB_SYNC_POOL, Sql) of
...
%%     Result = case mysql:fetch(?DB_SYNC_POOL, Query) of
%%         {data, #mysql_result{rows = Rows}} when Rows =:= [] -> null;
%%         {data, #mysql_result{rows = Rows}} -> [[R] | _] = Rows, R;
%%         {error, #mysql_result{error = Reason}} -> mysql_halt([Sql, Reason]);
%%         {error, _Reason} -> erlang:error({db_error, [_Reason]})
%%     end,
%%     ?profile(Sql),
...
%%     ?profile_start,
%%     Result = case mysql:execute(?DB_SYNC_POOL, Sql, Args) of
%%         {data, #mysql_result{rows = Rows}} when Rows =:= [] -> [];
%%         {data, #mysql_result{rows = Rows}} -> [R] = Rows, R;
%%         {error, #mysql_result{error = Reason}} -> mysql_halt([Sql, Reason]);
%%         {error, _Reason} -> erlang:error({db_error, [_Reason]})
...
%% %% @eg. batch_insert(<<"INSERT INTO `test` (id, num) VALUES ">>, [[1, 100], [2, 200]])
...
%% 
%% tx(Fun, Timeout) ->
%%     case mysql:transaction(?DB_SYNC_POOL, Fun, Timeout) of
%%         {atomic, Result} ->
%%             {ok, Result};
...
%% %% 返回数据库连接信息
%% conn_info() ->
%%     {status, _Pid, {module, _Mod}, [_PDict, _SysState, _Parent, _Debug, FmtMisc]} = sys:get_status(mysql_dispatcher),
...
%% 
%% test(sys) ->
%%     _D1 = db:get_one(<<"select name from role where id=10017 limit 1">>), ?DEBUG("get_one/1[D1]:~w", [_D1]),
%%     _D2 = db:get_one(<<"select name from role where id=~s limit 1">>, [10017]), ?DEBUG("get_one/2[D2]:~w", [_D2]),
%%     _D3 = db:get_one(<<"select id from role where name='传说哥' limit 1">>), ?DEBUG("get_one/1[D3]:~w", [_D3]),
%%     _D4 = db:get_one(<<"select id from role where name=~s limit 1">>, [<<"传说哥">>]), ?DEBUG("get_one/2[D4]:~w", [_D4]),
%% 
%%     _D5 = db:get_row(<<"select * from role where id=10017 limit 1">>),
%%     ?DEBUG("get_row/1[D5]:~w", [_D5]),
%% 
%%     _D6 = db:get_row(<<"select * from role where id=~s limit 1">>, [10017]),
%%     ?DEBUG("get_row/2[D6]:~w", [_D6]),
%% 
%%     _D7 = db:get_row(<<"select * from role where name=~s limit 1">>, [<<"传说哥">>]), ?DEBUG("get_row/2[D7]:~w", [_D7]),
%% 
%%     _D8 = db:get_row(<<"select * from role where name='传说哥' limit 1">>), ?DEBUG("get_row/1[D8]:~w", [_D8]),
%% 
%%     _D9 = db:get_all(<<"select id, name from role where id=10017 or id=10018 or id=10019 or id=234232">>), ?DEBUG("get_all/1[D9]:~w", [_D9]),
%% 
%%     _D10 = db:get_all(<<"select id, name from role where id=~s or id=~s or id=~s or id=~s">>, [10017, 10018, 10019, 234232]), ?DEBUG("get_all/2[D10]:~w", [_D10]),
%% 
%%     _D11 = db:select_limit(<<"select id, name, lev from role">>, 0, 10), ?DEBUG("get_select_limit/3[D11]:~w", [_D11]),
%% 
%%     _D12 = db:select_limit(<<"select id, name, lev from role where id<~s">>, [100], 0, 10), ?DEBUG("get_select_limit/4[D12]:~w", [_D12]),
%% 
%%     _D13 = db:execute(<<"update role set lev=119 where id=10017">>), ?DEBUG("execute/1[D13]:~w", [_D13]),
%%     _D17 = db:get_one(<<"select lev from role where id=10017 limit 1">>), ?DEBUG("get_one/1[D17]:~w", [_D17]),
%%     _D14 = db:execute(<<"update role set lev=~s where id=~s">>, [100, 10017]), ?DEBUG("execute/2[D14]:~w", [_D14]),
%%     _D15 = db:get_one(<<"select lev from role where id=10017 limit 1">>), ?DEBUG("get_one/1[D15]:~w", [_D15]),
%%     _D16 = db:execute(<<"update role set lev=~s where id=~s">>, [100, 10017]), ?DEBUG("execute/2[D16]:~w", [_D16]),
%%     ok.
