...
handle_call({query, Query, Timeout}, _From, State) ->
    ...
    handle_query_call_reply(Recs, Query, State1, #db_result{});
...
execute_stmt(Stmt, Args, Timeout, State = #state{socket = Socket}) ->
    ...
    handle_query_call_reply(Recs, Stmt#prepared.orig_query, State1, #db_result{}).
...
handle_query_call_reply([], _Query, State, #db_result{rows = Rows} = DbResult) ->
    Reply = 
        case Rows of
            []                    -> DbResult;
            [{_ColumnNames, _Rows}] -> DbResult;
            [_|_]                 -> DbResult#db_result{rows = lists:reverse(Rows)}
        end,
    {reply, Reply, State};
handle_query_call_reply([Rec|Recs], Query, State, #db_result{rows = Rows} = DbResult) ->
    case Rec of
        ...
        #ok{affected_rows = AffectRows, insert_id = InsertId} ->
            NDbResult = DbResult#db_result{affected_rows = AffectRows, insert_id = InsertId},
            handle_query_call_reply(Recs, Query, State, NDbResult);
        #resultset{rows = AddRows} ->
            NDbResult = DbResult#db_result{rows = AddRows ++ Rows},
            handle_query_call_reply(Recs, Query, State, NDbResult);
